...
// let n: null = null;
let nu: number = 4;
// nu = undefined; // strict 为true 开启了严格模式，需要去 tsconfig 里把 strictNullChecks 设置为 false, strictNullChecks 为 true 是只能赋值给 void 和自身
let v: void;

// 开启了 strictNullChecks
v = undefined; // 可以吧 undefined 赋值给 void 类型
v = null; // 不能把 null 赋值给 void 类型，但是官方文档上说是可以的，不清楚什么原因
...
// unknown 类型
// 任何类型都可以赋值给 unknown 类型，但是 unknown 类型除了赋值给 any 和它本身之外，不可以赋值给其他类型。所以，unknown 比 any 更安全。
